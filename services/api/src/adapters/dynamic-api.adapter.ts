 ...
 *
 * Key advantage over Crossmint: signTypedData computes the EIP-712
 * hash locally (via Viem) before MPC signing. The domain object never
 * reaches the server, so there's no validation of verifyingContract.
 * This allows signing Polymarket's ClobAuthDomain which omits it.
 *
...
export class DynamicApiAdapter implements DynamicServerWalletAdapter {
  ...
  async createWallet(userId: string): Promise<{ walletId: string; address: string }> {
    ...
    try {
      ...
      if (msg.includes('Multiple wallets') || msg.includes('Error creating evm wallet')) {
        // Wallet already exists in Dynamic environment — fetch it via REST API
        return this.getExistingWallet();
      }
      throw error;
    }
  }
  ...
  /**
   * Fetch the first existing wallet via the SDK's getWallets() method.
   * Used as a fallback when createWalletAccount() fails because a wallet
   * already exists in the environment.
   */
  private async getExistingWallet(): Promise<{ walletId: string; address: string }> {
    const client = await this.getClient();
    const wallets = await (client as any).getWallets() as Array<{
      walletId: string;
      accountAddress: string;
    }>;
    const wallet = wallets?.[0];
    if (!wallet) {
      throw new DynamicApiError('API_ERROR', 'No wallets found via SDK getWallets()', 404);
    }
  ...
  async getWallet(walletId: string): Promise<{ walletId: string; address: string; status: string }> {
    ...
    // not occur. If called, we look up via the SDK's getWallets().
    const client = await this.getClient();
    const wallets = await (client as any).getWallets() as Array<{
      walletId: string;
      accountAddress: string;
    ...
    return { walletId: wallet.walletId, address: wallet.accountAddress, status: 'active' };
  }
}
